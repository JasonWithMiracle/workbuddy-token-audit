...
> ### ⚠️ 本项目是 Vibe Coding 的产物
>
> 代码由 **AI 辅助生成**，人工主要负责需求定义、口径拍板与结果验收，**未做逐行代码审查**。
>
> 项目配有 46 个单元测试、独立实现复算校验、三平台 CI 与真实数据验证，但**使用前请自行评估**。
>
> 本项目按「原样」提供，**不附带任何担保**。详见下方 [关于本项目（Vibe Coding 声明）](#关于本项目vibe-coding-声明)。
...
**本项目是 Vibe Coding 的产物。** 在此如实说明其开发方式与质量边界，供你判断是否采用。

### 怎么开发的

- **代码主要由 AI 助手生成** —— 包括审计引擎、计价模块、报告生成器、测试、文档与 CI 配置
- **人工（项目发起者）主要负责**：需求定义、计价口径决策、方案拍板、结果验收，以及真实环境下的端到端验证
- **未做逐行人工代码审查** —— 因此可能存在人工审查本可发现的缺陷、冗余写法或非惯用实现

### 已有的质量保障

尽管开发方式如此，本项目并非「随手生成即发布」：

| 保障手段 | 说明 |
|---|---|
| **46 个单元测试** | 覆盖价格匹配优先级、峰谷判定（含调休日与法定假日）、分段判档、缓存分档公式、价格表与日历的数据契约 |
| **独立实现复算校验** | `verify_cost.py` 用与主引擎**不同的实现**重算全部请求，两者必须一致；实测 4 万条请求零不一致 |
| **三平台 CI** | ubuntu / windows / macos × Python 3.8 / 3.11 / 3.13，共 **9 个组合全部通过** |
| **提交前敏感扫描** | `check_privacy.py` 防止本机绝对路径或产出数据进入版本库 |
| **真实数据验证** | 在真实环境（Windows + 4 万条请求 + 49 天数据）跑通，关键数字经交叉核对 |

### 你该怎么看待它

- **逻辑性强的部分相对可靠**：计价公式、判档规则、缓存分档有单元测试与双实现交叉验证兜底
- **请自行评估这些方面**：异常处理、边界条件、跨平台细节、性能表现 —— 这些是 AI 生成代码常见的薄弱处
- **如果要用于关键决策**：建议先通读核心计价逻辑 `scripts/pricing_core.py`（仅约 120 行，是最需要看懂的部分）

### 不提供担保

本项目按 MIT 许可证「**按原样**」（AS IS）提供，不附带任何明示或暗示的担保，包括但不限于适销性与特定用途适用性。
**使用风险由使用者自行承担。**
